...
using NLQueryApp.Core.Utilities;
...
public abstract class BaseLlmService : ILlmService
{
    ...
    public async Task<LlmQueryResponse> GenerateSqlQueryAsync(LlmQueryRequest request)
    {
        ...
        // Note: request now contains QueryLanguage instead of DataSourceType
        var systemPrompt = SystemPrompt.CreateSystemPrompt(
            request.DatabaseSchema, 
            request.SchemaContext, 
            request.QueryLanguage,     // Changed from DataSourceType
        ...
        return await _retryHandler.ExecuteWithRetryAsync(async () =>
        {
            ...
            return ParseLlmResponse(content); // Removed dataSourceType parameter
        }, GetServiceName());
    }

    public Task<string> GenerateTitleAsync(string userQuestion)
    {
        // This method is deprecated and should not be used directly
        // Title generation should go through IDataSourceManager.GenerateTitleAsync
        _logger.LogWarning("GenerateTitleAsync called directly on LLM service. Use IDataSourceManager.GenerateTitleAsync instead.");
        return Task.FromResult(TitleGenerationUtility.GenerateFallbackTitle(userQuestion, "Database"));
    }
...
";
        ...
        // Updated to use QueryLanguage instead of DataSourceType
        return $"Convert the following question to a {request.QueryLanguage} query: {request.UserQuestion}";
    }

    protected LlmQueryResponse ParseLlmResponse(string content)  // Removed dataSourceType parameter
    {
        try
        {
            ...
            // Try sql first (most common), then any code block
